...
    // --- Añadido explícitamente por si hay pagos huérfanos ---
    await prisma.pago.deleteMany(); 
    await prisma.resenia.deleteMany();
    await prisma.alquiler.deleteMany();
    await prisma.turno.deleteMany();
  	await prisma.horarioDeshabilitado.deleteMany(); // Añadido para limpiar horarios deshabilitados
...
        data: { ...data, password: hashedPassword, rol: Rol.CLIENTE, suspendido: false }
      });
...
        data: { ...data, password: hashedPassword, rol: Rol.DUENIO, suspendido: false }
      });
      duenios.push(duenio);
    }
...
    console.log('⏰ Creando cronogramas com horarios puntuales...');
    
...
    const cronogramasData = []; // <--- *** INICIO DEL CAMBIO ***
    
...
          }
          
          // En lugar de 'await create', lo añadimos al array
          cronogramasData.push({
...
    // Ahora creamos todos los cronogramas en un solo lote
    console.log(`   Creando ${cronogramasData.length} registros de cronograma...`);
    await prisma.horarioCronograma.createMany({
      data: cronogramasData
    });
    // <--- *** FIN DEL CAMBIO ***

    // 12. Crear turnos disponibles para los próximos 7 días
    console.log('🎯 Creando turnos disponibles (7 días)...');
    
    // Volvemos a leer los cronogramas que acabamos de crear
...
    }
  
    // 13. Marcar 25% de turnos como ocupados (Crear Alquileres y Pagos)
    console.log('📅 Marcando 25% de turnos como ocupados y creando pagos...');
    
...
    const turnosSeleccionados: { turnoId: number, clienteId: number, precio: number }[] = [];
...
        precio: turnoRandom.precio // Guardar precio para el pago
      });
    }
...
    // Crear alquileres y pagos en lotes de 50 (secuencialmente)
...
          data: {
            // Si el turno es en el pasado, FINALIZADO, si no, PROGRAMADO
            estado: todosLosTurnos.find(t => t.id === turno.turnoId)!.fecha < hoy ? EstadoAlquiler.FINALIZADO : EstadoAlquiler.PROGRAMADO,
...
        // --- INICIO: Creación de Pago ---
        // Simular Pago (70% MP, 30% Efectivo)
        if (Math.random() < 0.7) {
...
              mpPreferenceId: `pref_mock_${alquiler.id}`,
              idPagoMp: `mp_mock_${Math.floor(Math.random() * 1000000000)}`,
              estadoPago: 'approved'
            }
          });
        } else {
          await prisma.pago.create({
            data: {
            	metodoPago: MetodoPago.TRANSFERENCIA, // <-- ARREGLADO
            	monto: turno.precio,
            	alquilerId: alquiler.id,
            	estadoPago: 'paid'
            }
...
Boolean(1)
        fechaAleatoria.setDate(hoy.getDate() - Math.floor(Math.random() * 90) - 1); // -1 para asegurar que sea en el pasado
        
...
        reservado: true,
        fecha: {
          lt: hoy // Asegurarse de que sean turnos del pasado
        }
      },
      orderBy: { id: 'desc' },
      take: totalResenias
...
    // Crear alquileres, pagos y reseñas (secuencialmente)
...
        // 2. Crear Pago (70% MP, 30% Efectivo)
        if (Math.random() < 0.7) {
          await prisma.pago.create({
            data: {
          	metodoPago: MetodoPago.MERCADOPAGO,
...
    console.log(`   - ${totalResenias} reseñas (10 por cancha)`);
    console.log(`   - ${pagosCreados} pagos (para reseñas y turnos ocupados)`);
    console.log(`   - Cronogramas con horarios puntuales`);
