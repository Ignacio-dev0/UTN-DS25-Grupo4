...
// Importar todos los servicios de usuario
...
/**
 * Obtiene los datos del usuario actualmente autenticado (basado en su JWT).
 * Se usa para refrescar el estado del AuthContext en el frontend.
 */
export async function getMe(req: Request, res: Response, next: NextFunction) {
    try {
        // El ID del usuario viene del middleware 'authenticate' (req.usuario.id)
        ...
        // Usar el new service
        const usuario = await usuarioService.getUsuarioParaContext(usuarioId);
        if (!usuario) {
            return res.status(404).json({ error: 'Usuario no encontrado' });
        }
        
        res.json(usuario); // Devolver el objeto de usuario seguro
