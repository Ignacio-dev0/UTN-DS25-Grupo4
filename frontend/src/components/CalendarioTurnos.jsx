...
function CalendarioTurnos({ turnosDisponibles }) {
  ...
  const handleConfirmarReserva = () => {
    console.log('Reserva confirmada para:', turnoSeleccionado);
        ...
        {reservaConfirmada ? (
          <div className="bg-accent border-l-4 border-secondary text-primary p-4 rounded-md shadow-lg w-full max-w-lg" role="alert">
            <div className="flex items-center">
              ...
              <div>
                <p className="font-bold text-lg">¡Reserva Exitosa!</p>
                <p>Tu turno para el <strong>{turnoSeleccionado?.dia} a las {turnoSeleccionado?.hora}</strong> ha sido confirmado.</p>
              </div>
            </div>
          </div>
        ) : (
