...
export const crearReseniaSchema = z.object({
  body: z.object({
    descripcion: z.string().min(10, "La descripción debe tener al menos 10 caracteres").max(500, "La descripción no puede exceder 500 caracteres"),
    puntaje: z.number().int().min(1, "El puntaje debe ser al menos 1").max(5, "El puntaje no puede ser mayor a 5"),
    alquilerId: z.number().int().positive("El ID del alquiler debe ser un número positivo")
  })
});
