...
function MiComplejoPage() {
    ...
    const { user, isAuthenticated } = useAuth(); // 2. ASUMIMOS QUE 'user' TIENE 'mpAccessToken'
            ...
            if (user?.rol === 'owner') {
                try {
                    ...
                    if (response.ok) {
                        ...
                        // MODIFICACIÓN: Si el estado es PENDIENTE (esperando MP), SÍ lo dejamos pasar
                        if (!complejoUsuario || (complejoUsuario.estado !== 'APROBADO' && complejoUsuario.estado !== 'PENDIENTE')) {
        ...
        const fetchComplejoData = async () => {
            try {
                ...
                if (user?.rol !== 'admin') {
                    ...
                    // MODIFICACIÓN: Si está PENDIENTE, lo dejamos cargar para mostrar el muro de MP
                    if (complejo.estado !== 'APROBADO' && complejo.estado !== 'PENDIENTE') {
    ...
    }, [complejoId, user?.rol, recargarCanchas]); // Saqué user de la dependencia para evitar loops
        ...
        if(isEditing) {
            try {
                ...
                if (response.ok) {
                    ...
                    window.alert('✅ Información del complejo actualizada correctamente'); // Usar window.alert si no hay modal
                ...
                window.alert(errorMessage); // Usar window.alert si no hay modal
            }
        }
    ...
    const handleDeleteCancha = async (canchaId) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                window.alert('Cancha eliminada correctamente');
            } else {
                throw new Error('Error al eliminar la cancha');
            }
        } catch (error) {
            console.error('Error eliminando cancha:', error);
            window.alert('Error al eliminar la cancha');
        }
    };
    ...
    const handleDisableCancha = async (canchaId) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                window.alert(`Cancha ${nuevaActiva ? 'habilitada' : 'deshabilitada'} correctamente`);
            } else {
                throw new Error('Error al actualizar el estado de la cancha');
            }
        } catch (error) {
            console.error('Error actualizando estado de cancha:', error);
            window.alert('Error al cambiar el estado de la cancha');
        }
    };
    ...
    const crearTurnos = async (canchaId, turnos) => {
        try {
            ...
            if (failed.length === 0) {
                window.alert(`${turnos.length} turnos creados correctamente`);
            } else {
                window.alert(`Se crearon ${responses.length - failed.length} de ${turnos.length} turnos`);
            }
        } catch (error) {
            console.error('Error creando turnos:', error);
            window.alert('Error al crear turnos');
        }
    };

    // Generar turnos automáticamente para los próximos 7 días
    const generarTurnosAutomaticos = async (canchaId) => {
        if (!window.confirm('¿Generar turnos automáticamente para los próximos 7 días (9:00 a 22:00)?')) return;
    ...
    let mpConectado = false; // <-- 3. AÑADIR VARIABLE DE ESTADO DE MP

    if (isAuthenticated && infoDelComplejo && user) { // <-- Asegurarse que user no sea null
        if (user.rol === 'admin') {
            puedeVerLaPagina = true;
            mpConectado = true; // El admin no necesita conectar MP para ver
        } 
        else if (user.rol === 'owner' && parseInt(user.id) === parseInt(infoDelComplejo.usuarioId)) {
            puedeVerLaPagina = true;
            // ¡Aquí está la magia!
            // Asumimos que tu AuthContext actualizó el 'user' con los campos de la DB
            // Tu schema usa 'mpAccessToken', así que lo buscamos.
    ...
    // --- 4. EL CAMBIO FINAL ---
    // Si puede ver la página, PERO no ha conectado MP, mostramos el Muro.
    if (!mpConectado) {
        return (
    ...
    // Si puede ver Y tiene MP conectado, muestra la página normal
    return (
        <div className="max-w-7xl mx-auto p-6 md:p-8 rounded-lg relative z-10">
            <h1 className="text-3xl font-bold font-lora text-gray-800 border-b border-gray-200 pb-4 mb-4">
                Mi Complejo
